...
class RandomSticker extends Component {
...
newPicture = () => {
    this.setState({url: "http://api.giphy.com/v1/stickers/random?api_key=dc6zaTOxFJmzC"});
    console.log(this.url);
    fetch(this.url)
        .then((data) => {
            return data.json();
...
render() {
    return (
        <div className="pageContainer">
            <Header headerText="Random Sticker" headerClass="sideHeader"/>
            {this.state.loaded ? (
